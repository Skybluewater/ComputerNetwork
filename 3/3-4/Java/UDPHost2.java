...
public class UDPHost2 implements Runnable {
	...
	private static boolean receiveFirstInfo = true;
	...
	public void fillAndSend(int method) throws Exception {
		...
		if (method == error) {// 模拟传输出错
			...
			System.out.println();
			DatagramPacket dp = new DatagramPacket(sendFrame, sendFrame.length, address, host1_Port);
			ds.send(dp);
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			is = new FileInputStream(new File("SendText2.txt"));
			os = new FileOutputStream(new File("ReceiveText1.txt"));
			ds = new DatagramSocket(host2_Port);
		...
		UDPHost2 send = new UDPHost2();
		UDPHost2 receive = new UDPHost2();
